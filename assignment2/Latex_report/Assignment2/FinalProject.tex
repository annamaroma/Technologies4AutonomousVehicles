...
For each processed frame, a multi-panel visualization is generated, including: the original image with ROI, lane detections, and YOLO outputs; the grayscale BEV with detected lanes; the ROI mask; the enhanced BEV image; the binary BEV image; and the column histogram used for lane initialization.
...
\section{Results}
per verificare la validità dell'algoritmo, il codice è stato messo alla prova in scenari diversi. 
Cominciando dal primo scenario, il numero 21. La figura ref ... mostra alcune detection dello scenario 21, notiamo come l'algoritmo abbia giustamente ricoosciuto le linee e identificate come dashed, nonostante fosse presente qualche volta del rumore dovuto ai tombini

per scenario 44 invece introduciamo degli oggetti, tipo notiamo che subito riconosce una persona in bicicletta e notiamo come quando la strada passa dalla salita alla discesa gestisce correttamente le linee e gli oggetti

scenario 2 fichissimo, i veicoli sono molti vicini alla macchina e qui emerge un'altra feature che ho implementato, il colore cambia in base alla vicinità. se la macchina è troppo vicina, il riquadro resta rosso

scenario 11 assurdo, addirittura riconosce le linee tratteggiate curve, quelle che segnalano i confini dell'incrocio

nel scenario 46 è proprio evidente come l'oggetto porti addirittura a limitare la roi, come se creasse un'ombra
dall'istogramma vediamo sempre un po' di rumore, magari si potrebbe andare avanti nello sviluppo di algoritmi migliori per togliere il rumore 
