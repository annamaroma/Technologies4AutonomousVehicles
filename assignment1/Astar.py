...
def astar(G, orig: int, dest: int, heuristic_type: str) -> Tuple[Optional[int], Optional[float], Optional[float]]:
    for node in G.nodes:
...
if __name__ == "__main__":
    import random
    #seed is necessary to ensure that the same random pairs of nodes are generated for each algorithm, so that the comparison between them is fair and consistent.
    random.seed(42) 
    ...
    for city in cities:
        print(f"\n--- Analysis for: {city} ---")
        G = prepare_graph(city)
        # Genera le stesse coppie ogni volta grazie al seed
        edges = generate_random_nodes(G, trials=trials) 
        ...
        for h_type in heuristics:
            ...
            for start, end in edges:
                # Modifica la chiamata per ricevere anche il costo (total_cost)
                it, total_cost, distance = astar(G, start, end, heuristic_type=h_type)
                
                if it is not None:
                    iterations.append(it)
                    times.append(total_cost)
                    distances.append(distance)
                    print(f"Start: {start}, End: {end}, Cost: {total_cost:.6f}, Iterations: {it}")

            avg_iterations = sum(iterations) / len(iterations) if iterations else 0
            avg_times = sum(times) / len(times) if times else 0
            avg_distance = sum(distances) / len(distances) if distances else 0

            print(f"AVG_ITERATIONS_ASTAR_{city}_{h_type}: {avg_iterations:.2f}")
            print(f"AVG_COST_ASTAR_{city}_{h_type}: {avg_times:.6f}")
            print(f"AVG_DISTANCE_ASTAR_{city}_{h_type}: {avg_distance:.2f}")

            save_path = f"results/{city.replace(' ', '_').replace(',', '')}_{h_type}.png"
            build_astar_collage(G, edges, heuristic_type=h_type, show=False, save_path=save_path)

        

    print("\nSimulazione A* completata.")
