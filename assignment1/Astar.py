...
def heuristic_computation(node, goal, maxspeed, heuristic_type):
    ...
    if heuristic_type == "Manhattan":
        ...
        delta_phi = phi1 - phi2
        delta_lambda = math.radians(x1 - x2)
...
def astar(orig, dest, heuristic_type="heuristicType"):
    ...
    h_n =heuristic_computation(orig, dest, vel_max_riferimento, heuristic_type) #f(n)= 0+h(n) for the start node is just the heuristic estimate to the goal
    ...
    while pq:
        ...
        for u, v, k, data in G.out_edges(node, data=True, keys=True):
            ...
            if tentative_g < G.nodes[neighbor]["g_score"]:
                ...
                #calculate h(n)
                h_n = heuristic_computation(neighbor, dest, vel_max_riferimento, heuristic_type)
...
for j in range(3):
    heuristic_type = heuristic_types[j]
    print(f"\nRunning A* with {heuristic_type} heuristic...")

    results = []
    for i in range(10):
        while True: #to ensure start and destination are different
